...
from utils.FigureUpdate import get_polar_ra, get_polar_re
...
def register_callbacks(app):
    ...
    def update_sliders_params(selected_name):
        ...
        # считаем максимальный радиус

        max_range = filtered_df["Range"].max()
        ...
        tstep = (
            ...
            else (tmax - tmin) / len(filtered_df)
        )
        marks = {}
        ...
        vstep = (
            (vmax - vmin) / 20
            if len(filtered_df) >= 20
            else (vmax - vmin) / len(filtered_df)
        )
        marks_vel = {}
    ...
    @app.callback(
        ...
        State("max-range", "data"),
    )
    def update_graphs(
        time_range, velocity_range, selectedpoints, filtered_dict, max_range
    ):
        filtered_df = pd.DataFrame(filtered_dict)
        if time_range is not None:
            filtered_df = filtered_df[
                (filtered_df["Time"] >= time_range[0])
                & (filtered_df["Time"] <= time_range[1])
            ]
        if velocity_range is not None:
            filtered_df = filtered_df[
                (filtered_df["Velocity"] >= velocity_range[0])
    ...
    def add_mapped_points(
        selection_xy,
        selection_xz,
        selectedpoints_state,
        filtered_dict,
        time_range,
        velocity_range,
    ):
        ...
        selectedpoints_xy = (
            [p["customdata"] for p in selection_xy.get("points", [])]
            if selection_xy
            else []
        )
        selectedpoints_xz = (
            [p["customdata"] for p in selection_xz.get("points", [])]
            if selection_xz
            else []
        )
    ...
    def plot_mapped_points(
        selectedpoints, filtered_dict, time_range, velocity_range, max_range
    ):
        filtered_df = pd.DataFrame(filtered_dict)
        if time_range is not None:
            filtered_df = filtered_df[
    ...
    def save_annotation(n_clicks, selected_indexes, df_dict):
        ...
        try:
            ...
            selected_ids = (
                df.loc[selected_indexes, "Id"].tolist() if selected_indexes else []
            )
            if selected_ids:
    ...
    def update_stats(
        ...
        if slider_range is not None:
            filtered_df = filtered_df[
                (filtered_df["Velocity"] >= slider_range[0])
